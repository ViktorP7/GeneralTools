---
title: "Irish MAP and VNTR 16-4-19"
...
## Visualising the phylogeny of Irish MAP

A tree file built from 109 Irish MAP sequences and sequences of international isolates featured in the 2016 Bryant paper was read into R. Corresponding locations, herds and VNTR types (where available) were pulled out from csv files and assigned to their appropriate tips in the tree. 
The tree was rooted with the node corresponding to the last common ancestor between MAP and M. avium, as in the Bryant paper. To examine Irish MAP, the subset with the respective strains was taken for plotting while retaining the structure and root. 
...
plot.phylo(irishOnlytree, edge.width = 0.2, font = 1, label.offset = 0.2, 
           ...
           align.tip.label = FALSE, type="phylogram", cex = 0.5, show.tip.label = FALSE)

tiplabels(pch = 18, col = tipColours,  cex = 0.8)
...
A "fan" version of the phylogeny was also constructed
...
## Discriminatory power of VNTR
To examine the discriminatory power of VNTR, a SNP distance matrix was created for 54 isolates for which metadata was available. The distances between a given isolate and all other isolates were categorised as being within the same VNTR or between VNTRs, and were plotted as a boxplot below. 
...
```
The boxplots looking at the SNP distances within the same VNTR and between different VNTRs don't appear to show any significant difference, meaning that in this particular case, VNTR typing cannot be used for accurate discrimination. It does not account for many SNPs within the same VNTR, but also classifies genetically similar isolates as different VNTR types. 
To confirm the validity of this approach, the mean SNP difference between the Between and Within groups was taken and compared to a distribution of 10,000 means where the VNTRs were randomly assigned to isolates. This scrambling examines if our observation can arise randomly by chance (and is therefore meaningless), or if it is significant.
```{r echo=FALSE}
...
The same process was repeated on medians instead of means and yielded a similar result, allowing for the conclusion that VNTR typing is not a suitable method of discrimination for Irish MAP isolates.
...
## Clustering by Herd and County

As with the VNTR work above, a similar process was carried out with herds and counties to determine if some sort of geographical clustering of MAP isolates was occuring.
```{r echo=FALSE}
...
Within herds, MAP isolates were genetically similar (with the exception of one outlier). Similarly, MAP isolates were likewise genetically similar within counties, supporting the argument that there is a degree of geographical clustering witnessed in the Irish MAP data.
...
Acquisition of more metadata will enable more accurate assessments of the usefulness of VNTR typing and clustering by county and herd. Initial indications from these 54 isolates show agreement with points raised in the literature about the inadequacy of VNTR typing for MAP.